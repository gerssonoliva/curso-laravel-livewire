<x-layouts.admin>
  ...
  <div class="bg-white px-6 py-8 rounded-lg shadow-lg">
    <form action="{{ route('admin.users.store') }}" method="post" class="space-y-4">
      ...
      <flux:input type="email" label="Correo" name="email" value="{{ old('email') }}" />
          ...
          @foreach ($roles as $role)
            <li>
              <label class="flex items-center gap-2">
                <input type="checkbox" name="roles[]" value="{{ $role->id }}" @checked(in_array($role->id, old('roles', []))) class="form-checkbox">
